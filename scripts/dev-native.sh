...
# execution. Reads .env.dev as the source of truth, then rewrites
# container hostnames (db, redis, worker, mailhog) to their localhost
# port mappings from docker-compose.dev.yml.
#
# Usage:
...
if [[ ! -f .env.dev ]]; then
    echo "ERROR: .env.dev not found. Run: make setup" >&2
    exit 1
fi

# Parse .env.dev as KEY=VALUE without shell expansion. We can't `source` it
# because values like `ZeroVPN <noreply@localhost>` contain `<`, which bash
# would interpret as a redirect. Docker compose parses .env files this way
# already, so we match its semantics here.
while IFS= read -r line || [[ -n "$line" ]]; do
    # Skip blanks and comment lines.
    [[ -z "${line// }" ]] && continue
    [[ "$line" =~ ^[[:space:]]*# ]] && continue
    # Split at the first '='. Lines without '=' are ignored.
    key="${line%%=*}"
    value="${line#*=}"
    [[ "$key" == "$line" ]] && continue
    # Trim a single layer of surrounding quotes if present.
    if [[ "$value" =~ ^\".*\"$ ]] || [[ "$value" =~ ^\'.*\'$ ]]; then
...
# Container hostnames → host localhost ports (matches docker-compose.dev.yml).
...
# Bind only to loopback when running natively — no reason to expose 8080
# on every interface on the host.
