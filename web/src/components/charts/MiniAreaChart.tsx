...
// Headroom above the window max so the trace never touches the top edge.
 ...
 * - The axis uses a sqrt scale so the full range always fits (no
 *   clipping at the top) while low-rate traffic still gets visible
 *   vertical movement — a linear max-fit would squash the typical band
 *   into a flat strip whenever one spike raised the ceiling.
...
export function MiniAreaChart({
  ...
  const yMax = useMemo(() => {
    // Both series share the y-axis so they stay visually comparable.
    let peak = 0
    for (const p of data) {
      if (p.rx > peak) peak = p.rx
      if (p.tx > peak) peak = p.tx
    }
    return Math.max(peak * PEAK_HEADROOM, MIN_DOMAIN)
  }, [data])
  ...
  return (
    <ResponsiveContainer width="100%" height={height}>
      <AreaChart data={data} margin={{ top: 1, right: 0, left: 0, bottom: 0 }}>
        ...
        <YAxis hide domain={[0, yMax]} scale="sqrt" />
        <Area
          type="linear"
          dataKey="rx"
          stroke={RX_COLOR}
